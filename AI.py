...
def main():
    ...
    def click():
        ...
        for i in range(len(numList)):
            if (int(numList[i])==1):
                print(txtList[i]+": interogative")
            if (int(numList[i])==2):
                print(txtList[i]+": auxillary verb")
            if (int(numList[i])==3):
                print(txtList[i]+": pronoun/noun")
            if (int(numList[i])==4):
                print(txtList[i]+": article")
            if (int(numList[i])==5):
                print(txtList[i]+": exclaimation")
            if (int(numList[i])==6):
                print(txtList[i]+": conjunction")
            if (int(numList[i])==7):
                print(txtList[i]+": article")
            if (int(numList[i])==8):
                print(txtList[i]+": verb")
            if (int(numList[i])==9):
                print(txtList[i]+": adverb")
            if (int(numList[i])==10):
                print(txtList[i]+": prepo")
            if (int(numList[i])==11):
                print(txtList[i]+": noun")
            if (int(numList[i])==12):
                print(txtList[i]+": adjective")
    ...
    usrIn.grid(row = 0, column = 0)
    usrIn.place(x=45,y=30)
    btn = Button(form, text="OK", width = 10,command= click)
    btn.grid(column=0, row=0)
    btn.place(x=155,y=55)
    txt = scrolledtext.ScrolledText(form,width=40,height=10)
